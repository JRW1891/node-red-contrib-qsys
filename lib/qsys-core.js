...
const CONFIG = {
    ...
    NOOP_INTERVAL: 30000,            // 30 seconds
    RECONNECT_BASE_DELAY: 1000,      // 1 second
    RECONNECT_MAX_RETRIES: 8,        // 8 retries = 2^8 = 256 seconds ~= 4.27 minutes max
    // Calculate max delay as BASE_DELAY * 2^MAX_RETRIES
    get RECONNECT_MAX_DELAY() { 
        return this.RECONNECT_BASE_DELAY * Math.pow(2, this.RECONNECT_MAX_RETRIES); 
    }
};
    ...
    function QsysCoreNode(n) {
        ...
        const logWithTimestamp = (message, isError = false) => {
            ...
            if (isError) {
                ...
                this.trace(formattedMessage); // Using trace for debug-level logging
            }
        };
        ...
        const reconnect = () => {
            ...
            const delay = CONFIG.RECONNECT_BASE_DELAY * Math.pow(2, retryExponent);
            ...
            if (!hasReachedMaxDelay) {
                if (reconnectAttempts >= CONFIG.RECONNECT_MAX_RETRIES) {
                    logWithTimestamp(`Reconnection backoff reached maximum retry count (${CONFIG.RECONNECT_MAX_RETRIES}, ${CONFIG.RECONNECT_MAX_DELAY/1000} seconds). Further reconnection attempts will continue silently.`);
        ...
        this.socket.on('connect', () => {
            logWithTimestamp('Socket connected', false);
            reconnectAttempts = 0;
